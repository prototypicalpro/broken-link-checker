...
export default class Link extends Map
{
	 ...
	 * @returns {Link}
	 */
	resolve(url, base, autoPrefix = false)
	{
		if (url != null)
		{
			// Parse or clone
 ...
 * @param {boolean|string} [autoprefix] Whether or not to automatically attempt adding https:// to the URL if parsing fails.
 * @returns {URL|null}
 */
const parseURL = (url=null, base, autoprefix=false) =>
{
	if (url !== null)
	{
		try
		...
		catch
		{
			if (typeof autoprefix === 'string') {
				try {
					url = new URL(`${autoprefix}${url}`, base || undefined)
				} catch {
					url = null;
				}
			}
